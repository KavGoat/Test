...
def _a_reviewed_drawing(window, tmp_path):
    """A drawing with one cloud on it, accepted, and a reply under it."""
    from PySide6.QtCore import QRectF
    from markforge.items.shapes import RectItem
    ...
    cloud = RectItem(kind="cloud")
    cloud.set_local_rect(QRectF(0, 0, 200, 120))
    cloud.subject = "Beam size"
    window.document.pages[0].frame.add_markup(cloud, QPointF(100, 100))
    window.view.scene().clearSelection()
    cloud.setSelected(True)
    window.set_markup_status("Accepted")
    cloud.add_reply("Confirmed on the structural drawings.", "K. Goat")
    return cloud


def test_a_status_is_signed_and_dated_by_whoever_set_it(window, tmp_path):
    """"Rejected" with nobody's name on it is not an answer to go back to."""
    cloud = _a_reviewed_drawing(window, tmp_path)
    assert cloud.status == "Accepted"
    assert cloud.status_by == "K. Goat"
    assert cloud.status_at, "and when they said it"


def test_a_review_survives_being_saved_and_reopened(window, tmp_path):
    cloud = _a_reviewed_drawing(window, tmp_path)
    saved = str(tmp_path / "reviewed.pdf")
    project_io.save_document(window.document, saved)

    reopened = Document()
    project_io.load_document(reopened, saved)
    written = [item for item in reopened.pages[0]._pending_items
               if item.get("uid") == cloud.uid]
    assert written, "the cloud should still be in the document"
    assert written[0]["status"] == "Accepted"
    assert written[0]["status_by"] == "K. Goat"
    assert [reply["text"] for reply in written[0]["replies"]] \
        == ["Confirmed on the structural drawings."]


def test_a_review_reaches_the_pdf_as_the_pdf_records_one(window, tmp_path):
    """As reply annotations, which is how Acrobat and Bluebeam record one."""
    from pypdf import PdfReader

    _a_reviewed_drawing(window, tmp_path)
    saved = str(tmp_path / "reviewed.pdf")
    project_io.save_document(window.document, saved)

    annotations = [entry.get_object()
                   for entry in (PdfReader(saved).pages[0].get("/Annots") or [])]
    replies = [entry for entry in annotations if "/IRT" in entry]
    assert len(replies) == 2, "the reply and the status both"
    assert all(str(entry["/RT"]) == "/R" for entry in replies)

    states = [entry for entry in replies if "/State" in entry]
    assert len(states) == 1
    assert str(states[0]["/State"]) == "Accepted"
    assert str(states[0]["/StateModel"]) == "Review"
    assert str(states[0]["/T"]) == "K. Goat"

    said = [entry for entry in replies if "/State" not in entry]
    assert str(said[0]["/Contents"]) == "Confirmed on the structural drawings."


def test_a_review_somebody_else_made_comes_in_as_a_review(window, tmp_path):
    """A drawing that has been through Bluebeam arrives with its review on it."""
    from markforge.io import pdfio

    reviewed = str(tmp_path / "theirs.pdf")
    _a_reviewed_drawing(window, tmp_path)
    project_io.save_document(window.document, reviewed)

    fresh = Document()
    pdfio.import_pages(fresh, reviewed, [0], pdfio.FIT_ORIGINAL, 96.0, at=0)
    clouds = [item for item in (fresh.pages[0]._pending_items or [])
              if item.get("subject") == "Beam size"]
    assert clouds, "their cloud should come in as a markup"
    assert clouds[0]["status"] == "Accepted"
    assert clouds[0]["status_by"] == "K. Goat"
    assert [reply["text"] for reply in clouds[0]["replies"]] \
        == ["Confirmed on the structural drawings."]
