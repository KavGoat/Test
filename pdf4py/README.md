...
python -m pdf4py                    # or: python -m pdf4py document.pdf
```

Python 3.10 or newer, PySide6 for the interface and
[PyMuPDF](https://pymupdf.readthedocs.io) for the PDF itself.
...
The strip on the left is one thumbnail per page; click one to go to it.
...
pdf4py/
  ...
  ui/pageview.py   the canvas: the page, the markup items, the two tools
  ui/pagelist.py   the page thumbnails
  ui/mainwindow.py menus, toolbar and the wiring between the two
```
...
- **The file is read into memory** and the handle closed, so a document can
  always be saved back over the file it was opened from.
- **Pages are rendered without their annotations**, and every markup is then
  ...
  makes a markup something you can pick up and drag rather than part of a
  picture.
- **Coordinates are display points** everywhere outside `document.py`: PDF user
  ...
  because PyMuPDF's `Annot.set_rect` re-applies the border padding each time and
  would walk the markup a point further with every drag.

## Tests

```bash
python -m pytest tests/test_pdf4py.py
```

They cover the document model — exact, drift-free moves, rotated pages, the
save round trip — and drive the window itself with synthetic mouse events.
