# MarkForge — Requirements and Bug Register
...
These are the user's own confirmations from the workbook, recorded when the
application also did calculations, spreadsheets and plots. They are left
exactly as they were confirmed; the sections naming withdrawn work are history,
not open requirements.
...
- [ ] **(new, amended)** This is a PDF markup editor and nothing else. Opening a PDF gives Bluebeam-style navigation, markup, measurement and Snapshot tools over the page. **Amended 2026-09-07**: the original wording asked for this as a *mode* alongside calculations; the calculations have since been withdrawn, so it is the whole application rather than one of its modes.
...
### 7. Markup tools — placement and interaction (Bluebeam parity)
...
- [ ] **(amended)** "Properties mode" (place a new copy using the last-used style/properties rather than an exact one-to-one duplicate) should only be selectable for a single markup object — for images, snapshots and groups it should be greyed out or simply not offered, since it doesn't make sense for those (41). **Amended 2026-09-07**: calc blocks and graphs removed from the exception list; they no longer exist.
...
- [ ] **(amended)** Pressing `G` while reviewing a PDF must reliably create a Snapshot from the selected PDF region. That snapshot must be saved into the document and remain visible after save, reopen, export and ordinary document editing.
...
- [ ] **(amended)** Bluebeam-style unified dock: Pages, Bookmarks, Toolsets/My Tools, Properties, and any other relevant panel should each be a togglable icon that can be dragged individually to dock on either the left or right side (45, 62).
...
- [ ] **(amended)** All shortcuts must be disabled while actively typing into a markup, **except** Ctrl+B/I/U which remain bold/italic/underline for text formatting (20, 131).
...
- [ ] **(amended)** Snapshot must capture PDF vector linework and markups without carrying through the page background; it must not include unrelated text markups unless those are explicitly selected for capture.
...
- [ ] **(amended)** While editing text, Ctrl+B/Ctrl+I/Ctrl+U format the selected text. Outside text editing they retain their global commands; formatting keys must never insert a page or trigger unrelated commands.
...
- [ ] **(amended)** Add recoverable document flattening: choose which content classes to flatten, retain recovery data by default, support individual-item flattening, and offer a Preferences setting to disable recoverability when deliberately producing an irreversible file. Flattening must behave the way it does on a Bluebeam PDF: flattened content becomes part of the page, so it is no longer selectable and no longer intercepts a click meant for something in front of or behind it. The flatten tool must not sit under a crosshair cursor — it is not a drawing tool — and while it is active it must still be possible to pick out the things it is about to act on.
...
### 30. Asked for since the strip-down (2026-09-07)

Added as requirements when they arrived, per rule 3. Each carries what was
built against it; none of them is ticked, because completion is the user's to
mark.

- [ ] **(new)** Build the PDF engine in Python rather than wrapping somebody
  else's, taking PDF4QT as the model — "make the pdf4qt in python like the base
  engine and add the markups stuff and scale etc". *Built*: `markforge/pdf/`
  — objects, lexer, filters, object storage, reader, writer with incremental
  update, annotation model. Reads classic cross-reference tables and streams,
  object streams, Flate/LZW/ASCIIHex/ASCII85/RunLength with PNG and TIFF
  predictors, and recovers a file whose cross-reference table is wrong by
  scanning. Evidence: `tests/test_pdf_engine.py`, against real files from
  other people's software rather than files of its own making.
- [ ] **(new)** Saving a marked-up drawing must add to the drawing rather than
  rewrite it. *Built*: `markforge/io/pdfsave.py` writes an incremental update
  when the document is one PDF and its markups — the original bytes are the
  prefix of the saved file, the markups are appended as annotations. Anything
  that has to be painted (several sources, a dimmed page, a flattened markup, a
  running header or footer) is assembled as before. Evidence:
  `tests/test_format.py`.
- [ ] **(new)** Snapping must catch where two lines cross — grid lines meeting,
  a beam arriving at a column — which is the point a drawing is aimed at and
  the one nothing has a vertex for. *Built*: only the segments passing near the
  pointer are compared; lines that stop short of each other do not cross, and
  where they would have met is not offered. Evidence: four tests in
  `tests/test_usability.py`.
- [ ] **(new)** The markups list and the drawing are two views of one thing:
  picking a row picks the markup, picking a markup picks its row. *Built*, with
  the Layer column and sortable headings that went in beside it.
- [ ] **(new)** Ordering is what layers were kept for: bring front, send back,
  forward, backward. The page's own imported line work sits under everything
  drawn on it, is never dragged about, and "send to back" means behind the other
...
this register, so they are recorded here with what withdrew them rather than
removed. None of them is a completed requirement and none may be counted as one.

- **2026-09-07 — "Can you create a branch and recreate this app without any
  calculation stuff, no tables, no equation, no plot. Remove all cfx
  functionality and panels etc."** Everything in the old sections 2
  (calculation engine), 3 (blocks vs lines), 4 (equation editor) and 6
  (spreadsheet) goes, along with plots, the `.cfx` format, and every task that
  only made sense alongside them. The individual lines follow below.
- **2026-09-07 — "Status and replies not needed."** Markup review states
  (Accepted / Rejected / Cancelled / Completed) and per-markup reply threads,
  built and then removed.
- **2026-09-07 — "Layers not needed, only send to back etc needed."** The
  layers panel and per-layer show, lock and print. Bring front, send back,
  forward and backward stay and are the whole of what ordering means now.

### Withdrawn lines, as they were written
...
- Nothing is deleted. When the user withdraws a requirement, it moves to
	**Withdrawn** with the instruction that withdrew it, struck through and kept.
	A withdrawn item is not a completed one and must never be counted as one.
